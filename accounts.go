...
func handleAccountsCommand(client *Client, args []string) error {
	...
	if *inspectFlag != "" {
		return client.inspectAccount(*inspectFlag)
	}

	if *updateFlag != "" {
		// Build update request from flags
		return client.updateAccountFromFlags(*updateFlag,
			*peerLoginExpFlag,
...
func (c *Client) inspectAccount(accountID string) error {
...
func (c *Client) updateAccountFromFlags(accountID string,
	...
	if peerLoginExp != "" {
		seconds, err := parseDuration(peerLoginExp)
		if err != nil {
	...
	if peerInactivityExp != "" {
		seconds, err := parseDuration(peerInactivityExp)
		if err != nil {
	...
	updateResp, err := c.makeRequest("PUT", "/accounts/"+accountID, bytes.NewReader(bodyBytes))
	if err != nil {
		return err
	}
...
// printAccountUsage prints the usage information for the account command
func printAccountUsage() {
	fmt.Println("Usage: netbird-manage account [options]")
	fmt.Println("\nQuery Commands:")
	...
	fmt.Println("  --network-range <cidr>                  Network IP range (e.g., 100.64.0.0/10)")
	fmt.Println("  --jwt-groups-enabled <bool>             Enable JWT group claims")
	fmt.Println("  --jwt-groups-claim <claim>              JWT claim name for groups")
	fmt.Println("  --jwt-allow-groups <groups>             Comma-separated allowed groups")
	fmt.Println("  --groups-propagation-enabled <bool>     Enable groups propagation")
