...
func (s *Service) HandleNetworkCommand(args []string) error {
	...
	if *inspectResourceFlag {
		...
		return s.inspectNetworkResource(*networkID, *resourceID)
	}
	if *addResourceFlag != "" {
	...
	if *removeResourceFlag {
		...
		return s.removeNetworkResource(*networkID, *resourceID)
	}

	// Handle router operations
	if *listAllRoutersFlag {
		return s.listAllRouters()
	}
	if *listRoutersFlag != "" {
	...
	if *inspectRouterFlag {
		...
		return s.inspectNetworkRouter(*networkID, *routerID)
	}
	if *addRouterFlag != "" {
		if *peer == "" && *peerGroups == "" {
	...
	if *removeRouterFlag {
		...
		return s.removeNetworkRouter(*networkID, *routerID)
	}
...
func (s *Service) listNetworks(filterName string, outputFormat string) error {
	...
	if filterName != "" {
		...
		for _, net := range networks {
			if helpers.MatchesPattern(net.Name, filterName) {
...
func (s *Service) createNetwork(name, description string) error {
...
func (s *Service) inspectNetworkResource(networkID, resourceID string) error {
	...
	var resource models.NetworkResource
...
func (s *Service) removeNetworkResource(networkID, resourceID string) error {
	...
	// Ask for confirmation
	if !helpers.ConfirmSingleDeletion("network resource", resource.Name, resourceID, details) {
		return nil // User cancelled
	}
...
func (s *Service) inspectNetworkRouter(networkID, routerID string) error {
	...
	var router models.NetworkRouter
...
func (s *Service) removeNetworkRouter(networkID, routerID string) error {
	...
	// Ask for confirmation
	if !helpers.ConfirmSingleDeletion("network router", "", routerID, details) {
		return nil // User cancelled
	}
