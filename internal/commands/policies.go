...
func (s *Service) HandlePoliciesCommand(args []string) error {
	...
	// Enable policy
	if *enableFlag != "" {
		return s.togglePolicy(*enableFlag, true)
	}
...
func (s *Service) createPolicy(name, description string, enabled bool) error {
...
func (s *Service) createPolicyWithRule(name, description string, enabled bool, ruleConf *ruleConfig) error {
	...
	initialRule, err := s.buildRuleFromConfig(ruleName, ruleConf)
	if err != nil {
		return err
	}
	...
	reqBody := models.PolicyCreateRequest{
		...
		Description: description,
		Enabled:     enabled,
		Rules:       []models.PolicyRuleForWrite{*convertRuleToWrite(initialRule)},
	}
...
func convertRuleToWrite(rule *models.PolicyRule) *models.PolicyRuleForWrite {
	...
	return &models.PolicyRuleForWrite{
...
func (s *Service) editRule(policyID, ruleIdentifier string, config *ruleConfig) error {
	...
	if config.Destinations != "" {
		destGroups, err := s.resolveGroupIdentifiers(config.Destinations)
		if err != nil {
			return err
		}
...
func (s *Service) buildRuleFromConfig(ruleName string, config *ruleConfig) (*models.PolicyRule, error) {
	...
	if config.PortRange != "" {
		...
		rule.PortRanges = []models.PortRange{*portRange}
	}
...
func (s *Service) resolveGroupIdentifiers(identifiers string) ([]models.PolicyGroup, error) {
	if identifiers == "" {
		return []models.PolicyGroup{}, nil
	}
	...
	for _, part := range parts {
		...
		group, err := s.getGroupByNameOrID(part)
		if err != nil {
			return nil, fmt.Errorf("failed to resolve group '%s': %v", part, err)
		}

		groups = append(groups, models.PolicyGroup{
			ID:   group.ID,
			Name: group.Name,
		})
	}
...
func (s *Service) getGroupByNameOrID(identifier string) (*models.GroupDetail, error) {
	...
	return s.getGroupByName(identifier)
}

// parsePortRange parses a port range string like "6000-6100"
func parsePortRange(rangeStr string) (*models.PortRange, error) {
...
func formatPorts(ports []string, portRanges []models.PortRange) string {
...
func cleanRulesForUpdate(rules []models.PolicyRule) []models.PolicyRuleForWrite {
	...
	for i, rule := range rules {
		...
		cleaned[i] = models.PolicyRuleForWrite{
